...
Process Manager  ──▶  [Worker Group(s)]  ◀── this component
                              ...
                              ├── Import storages (NFS/PVC, read-write)
...
| per-entry `mountPath` | `imports[]` PVC/NFS | **ReadWriteMany** | Staging areas — source files for import jobs; workers process and clean up. Multiple import storages supported. |
...
Each worker group gets the shared roots (`akubraConfig`, `solrConfig`, `auth.keycloak`). Workers do **not** connect to any database — all task state is managed by the Process Manager. The `import.directory` property is auto-generated by the chart as a comma-joined list of all `storages.imports[].mountPath` values. Worker-specific properties (for example image-server conversion flags) go in `workerGroups[].config.configurationPropertiesExtra`.
...
3. Worker executes task (reads/writes Akubra + import storages)
...
| **Import storages** | POSIX filesystem RW | Read source files for import; clean up after processing (one or more volumes) |
